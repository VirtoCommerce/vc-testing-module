...
class ShippingDetailsSection(Component):
    ...
    def shipping_address_section(self) -> ShippingAddressSection:
        return ShippingAddressSection(root=self._root.locator("[data-test-id='shipping-address-section']"))

    @property
    def pickup_location_section(self) -> PickupLocationSection:
        return PickupLocationSection(root=self._root.locator("[data-test-id='pickup-location-section']"))
