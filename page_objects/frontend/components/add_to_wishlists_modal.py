...
class AddToWishlistsModal(Component):
    ...
    def list_checkbox(self, list_id: str) -> Locator:
        return self._root.locator(f"label:has([data-test-id='wishlist-modal-list-checkbox-{list_id}'])")

    def list_with_product_checkbox(self, list_id: str) -> Locator:
        return self._root.locator(f"label:has([data-test-id='wishlist-modal-list-with-product-checkbox-{list_id}'])")
