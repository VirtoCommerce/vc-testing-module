...
class JsonResolver:
    ...
    def _validate_directories(self, manifest: list[ManifestEntry]) -> None:
        ...
        empty = [
            name
            for name in manifest_names & existing_dirs
            if not list((self._data_dir / name).glob("*.json"))
        ]
        if empty:
            self._logger.warning(
                f"[yellow]Directories contain no JSON files:[/yellow] {', '.join(sorted(empty))}"
            )
        ...
        if isinstance(value, str):
            def replace(m: re.Match[str]) -> str:
                ...
                if env_value is None:
                    self._logger.warning(
                        f"[yellow]\\[{entity_name}] ENV variable not set:[/yellow] {var_name}"
                    )
                    return m.group(0)
                return env_value
