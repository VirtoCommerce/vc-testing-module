...
class CartPage:
    ...
    def expect_line_item_total(
        self, product_name: str, price: float, quantity: int, line_item_number1: int, line_item_number2: int
    ):
        ...
        assert (
            abs(actual_amount - expected_total) < 0.01
        ), f"Expected total {expected_total} for product {product_name}, but got {actual_amount}"
    ...
    def expect_selected_items_count(self, expected_count: int):
        ...
        assert (
            selected_checkboxes == expected_count
        ), f"Expected {expected_count} selected items, but found {selected_checkboxes}"
