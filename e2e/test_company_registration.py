...
from e2e.pages.testData.user_data import (
    get_random_user,
    PASSWORD_TEST_CASES,
    generate_valid_password,
    get_random_invalid_email,
...
def test_invalid_password_combinations(registration_page: RegistrationPage):
    ...
    for case, password in PASSWORD_TEST_CASES.items():
        ...
        if case == "no_lowercase":
            ...
            assert (
                registration_page.is_error_visible()
...
def test_password_mismatch(registration_page: RegistrationPage):
    ...
    for case in test_cases:
        ...
        for expected_error in case["expected_errors"]:
            assert any(
                expected_error in error_text for error_text in error_texts
...
def test_valid_email_formats(registration_page: RegistrationPage):
    ...
    valid_emails = [
        {"email": "test@playwright.com", "description": "Standard email"},
        {"email": "test.name@playwright.com", "description": "Email with dot"},
        {"email": "test+label@playwright.com", "description": "Email with plus sign"},
        {"email": "test@sub.playwright.com", "description": "Email with subdomain"},
    ]
    ...
    for case in valid_emails:
        ...
        assert not registration_page.is_error_visible(), f"Error shown for valid email: {case['email']}"
...
def test_invalid_email_formats(registration_page: RegistrationPage, config):
    ...
    invalid_emails = [
        ...
        {"email": "invalid.email", "description": "Missing @ symbol", "expected_error": SIGNUP_ERROR["email_invalid"]},
        {
            "email": "@playwright.com",
            "description": "Missing local part",
            "expected_error": SIGNUP_ERROR["email_invalid"],
        },
        {"email": "test@", "description": "Missing domain", "expected_error": SIGNUP_ERROR["email_invalid"]},
        {"email": "test@.com", "description": "Invalid domain", "expected_error": SIGNUP_ERROR["email_invalid"]},
        {"email": "", "description": "Empty email", "expected_error": SIGNUP_ERROR["required_field"]},
        {
    ...
    for case in invalid_emails:
        ...
        assert registration_page.is_error_visible(), f"No error shown for invalid email: {case['email']}"
        ...
        assert (
            case["expected_error"] in error_text
