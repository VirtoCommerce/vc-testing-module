...
class CategoryPage(MainLayout):
    ...
    def url(self) -> str:
        return (
            f"{self._global_settings.frontend_base_url}/{self._path}?sort={self._sort}"
        )
    ...
    def view_switcher(self) -> CategoryViewSwitcher:
        return CategoryViewSwitcher(
            root=self._page.locator("[data-test-id='view-switcher']")
        )
    ...
    def price_checkboxes_filter(self) -> CheckboxesFilter:
        return CheckboxesFilter(
            root=self._page.locator("[data-test-id='filter-price']")
        )
    ...
    def scroll_to_product(self, sku: str) -> ProductCard:
        ...
        while True:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Product with SKU '{sku}' not found on category page"
                )
