...
def test_health_check(global_settings: GlobalSettings):
...
def test_health_check_status(global_settings: GlobalSettings):
    with allure.step("GET /health"):
        response = requests.get(
            f"{global_settings.backend_base_url}/health",
            timeout=global_settings.requests_timeout,
    ...
    with allure.step("Verify status"):
        ...
        assert "healthy" in text or response.status_code == 200


@pytest.mark.restapi
@allure.feature("Health Check (REST API)")
@allure.title("Health check — Elasticsearch connectivity")
def test_health_check_elastic(rest_client, backend_base_url: str):
