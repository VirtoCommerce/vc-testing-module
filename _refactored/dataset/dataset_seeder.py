...
class DatasetSeeder:
    ...
    def _fetch_installed_modules(self) -> set[str]:
        ...
        if not isinstance(response, list):
            ...
            return set()
        module_ids = {
            m["id"] for m in response if "id" in m and m.get("isInstalled") is True
        }
        self._logger.info(f"Fetched {len(module_ids)} installed module(s)")
        return module_ids

    def _filter_by_installed_modules(
        self, item: dict[str, Any], entity_name: str
    ) -> dict[str, Any]:
        ...
        for key, value in item.items():
            if isinstance(value, list) and value and isinstance(value[0], dict):
                ...
                kept = [
                    v for v in original
                    if "moduleId" not in v
                    or v["moduleId"] in self._installed_modules
                ]
                skipped = [
                    v for v in original
                    if "moduleId" in v
                    and v["moduleId"] not in self._installed_modules
                ]
                for v in skipped:
    ...
    def seed(self, names: list[str] | None = None) -> None:
        ...
        for entry in entries:
            ...
            if not items:
                self._logger.warning(
                    f"[yellow]No data for entity:[/yellow] {entry.name}"
                )
                continue
        ...
        if entry.payload_type == "array":
            ...
            for item in items:
                ...
                if self._has_unresolved_env(item):
                    self._logger.warning(
                        f"[yellow]\\[{entry.name}] Skipping {label or 'item'}:[/yellow] "
                        f"unresolved ENV placeholders"
                    )
                    continue
                endpoint = self._resolve_endpoint(
                    endpoint=entry.endpoint, item=item, entity_name=entry.name
                )
                url = f"{base_url}{endpoint}"
                prefix = (
                    f"Seeding \\[{entry.name}: {label}]"
                    if label
                    else f"Seeding \\[{entry.name}]"
                )
                self._call(
                    method=entry.method,
                    url=url,
    ...
    def _call(
        ...
        try:
            ...
            self._logger.info(
                f"{log_prefix} {method} {url} [green]DONE[/green] \\[{elapsed:.2f}s]"
            )
        except Exception as e:
            elapsed = time.perf_counter() - start
            self._logger.error(
                f"{log_prefix} {method} {url} [red]FAILED[/red] \\[{elapsed:.2f}s]: {e}"
            )
