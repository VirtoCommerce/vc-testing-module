...
def test_contact_change_role(graphql_client: GraphQLClient) -> None:
    ...
    try:
        with allure.step(f"Change role to '{_TARGET_ROLE}' for contact {_TARGET_CONTACT_ID}"):
            result = contact_ops.change_organization_contact_role(member_id=_TARGET_CONTACT_ID, role_ids=[_TARGET_ROLE])
