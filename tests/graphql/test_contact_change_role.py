...
def test_contact_change_role(graphql_client: GraphQLClient, ctx: Context) -> None:
    ...
    result = contact_ops.change_organization_contact_role(user_id=user_id, role_ids=[_TARGET_ROLE])
    ...
    contact_ops.change_organization_contact_role(user_id=user_id, role_ids=[_ORIGINAL_ROLE])
