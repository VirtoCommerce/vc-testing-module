...
def test_user_invite(graphql_client: GraphQLClient, ctx: Context, global_settings: GlobalSettings) -> None:
    ...
    try:
        ...
        def _find_invited_contact():
            ...
            return next(
                (
                    c
                    for c in contacts
                    if c.status == _STATUS_INVITED and any(a.user_name == email for a in c.security_accounts)
                ),
                None,
            )
        ...
        assert invited_contact is not None, f"Invited contact '{email}' not found after polling"
