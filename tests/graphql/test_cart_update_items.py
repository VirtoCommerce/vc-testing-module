...
@pytest.mark.graphql
@pytest.mark.delete_cart_after
def test_cart_update_variation_items(graphql_client: GraphQLClient, ctx: Context) -> None:
    cart_ops = CartOperations(client=graphql_client)
    ...
    assert has_line_item(cart.items, product_id=_VARIATION_ID, quantity=_INITIAL_QUANTITY)
    ...
    assert has_line_item(cart.items, product_id=_VARIATION_ID, quantity=_TARGET_QUANTITY)
