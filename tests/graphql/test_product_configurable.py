...
def test_product_configurable(graphql_client: GraphQLClient, ctx: Context) -> None:
    ...
    with allure.step(f"Get configuration for product {_PRODUCT_ID}"):
    ...
    with allure.step("Build selection picking the first option of each section"):
        selected_sections = []
    ...
    with allure.step(f"Create configured line item for {_PRODUCT_ID}"):
    ...
    with allure.step(f"Verify line item references product {_PRODUCT_ID}"):
