...
def test_cart_unselect_item(graphql_client: GraphQLClient, ctx: Context, with_cart: Cart) -> None:
...
@pytest.mark.graphql
@pytest.mark.with_cart([(_PRODUCT_ID_1, 1), (_PRODUCT_ID_2, 1)])
def test_cart_select_item(graphql_client: GraphQLClient, ctx: Context, with_cart: Cart) -> None:
    cart_ops = CartOperations(client=graphql_client)
