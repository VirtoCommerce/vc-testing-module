...
@pytest.mark.graphql
@pytest.mark.skip
def test_pickup_locations_all(graphql_client: GraphQLClient, ctx: Context, global_settings: GlobalSettings) -> None:
    ...
    assert all(getattr(loc.address, field) for loc in all_locations for field in _ADDRESS_FIELDS)


@pytest.mark.graphql
@pytest.mark.skip
def test_pickup_locations_by_keyword(graphql_client: GraphQLClient, ctx: Context) -> None:
    ops = PickupLocationOperations(client=graphql_client)
