...
def test_orders_filter_by_status_new(graphql_client: GraphQLClient, ctx: Context) -> None:
...
@pytest.mark.graphql
@pytest.mark.with_user(_MAINTAINER)
def test_orders_filter_by_status_completed(graphql_client: GraphQLClient, ctx: Context) -> None:
    orders = OrderOperations(client=graphql_client).get_organization_orders(
        organization_id=ctx.organization_id,
...
@pytest.mark.graphql
@pytest.mark.with_user(_MAINTAINER)
def test_orders_filter_by_status_pending(graphql_client: GraphQLClient, ctx: Context) -> None:
    orders = OrderOperations(client=graphql_client).get_organization_orders(
        organization_id=ctx.organization_id,
...
@pytest.mark.graphql
@pytest.mark.with_user(_MAINTAINER)
def test_orders_filter_by_status_payment_required(graphql_client: GraphQLClient, ctx: Context) -> None:
    orders = OrderOperations(client=graphql_client).get_organization_orders(
        organization_id=ctx.organization_id,
...
@pytest.mark.graphql
@pytest.mark.with_user(_MAINTAINER)
def test_orders_filter_by_status_ready_for_pickup(graphql_client: GraphQLClient, ctx: Context) -> None:
    orders = OrderOperations(client=graphql_client).get_organization_orders(
        organization_id=ctx.organization_id,
...
def test_orders_filter_by_date(graphql_client: GraphQLClient, ctx: Context) -> None:
    ...
    assert all(
        datetime.fromisoformat(o.created_date.replace("Z", "+00:00")) >= anchor - timedelta(days=7)
