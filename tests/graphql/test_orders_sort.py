...
def test_organization_orders_sort_by_date(graphql_client: GraphQLClient, sort: str) -> None:
...
@pytest.mark.with_user(_ADMINISTRATOR)
def test_organization_orders_sort_by_total(graphql_client: GraphQLClient, sort: str) -> None:
    orders = OrderOperations(client=graphql_client).get_organization_orders(sort=sort)
