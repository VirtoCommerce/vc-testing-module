...
def _assert_addresses(locations) -> None:
    ...
    assert all(getattr(loc.address, field) for loc in locations for field in _ADDRESS_FIELDS)


@pytest.mark.graphql
@pytest.mark.skip
def test_product_pickup_locations_all_types(graphql_client: GraphQLClient, ctx: Context) -> None:
...
@pytest.mark.graphql
@pytest.mark.skip
def test_product_pickup_locations_today(graphql_client: GraphQLClient, ctx: Context) -> None:
    ops = PickupLocationOperations(client=graphql_client)
    locations = _get_locations(ops, _PRODUCT_TODAY, ctx)

    today_locations = [loc for loc in locations if loc.availability_type == _AVAILABILITY_TODAY]
    assert len(today_locations) > 0
    assert all(loc.available_quantity is not None and loc.available_quantity > 0 for loc in today_locations)
    _assert_addresses(locations)


@pytest.mark.graphql
@pytest.mark.skip
def test_product_pickup_locations_multiple_ffc(graphql_client: GraphQLClient, ctx: Context) -> None:
    ops = PickupLocationOperations(client=graphql_client)
    ...
    transfer_locations = [loc for loc in locations if loc.availability_type == _AVAILABILITY_TRANSFER]
...
@pytest.mark.graphql
@pytest.mark.skip
def test_product_pickup_locations_no_track_inventory(graphql_client: GraphQLClient, ctx: Context) -> None:
    ops = PickupLocationOperations(client=graphql_client)
...
@pytest.mark.graphql
@pytest.mark.skip
def test_product_pickup_locations_defined_ffc_berlin_billund(graphql_client: GraphQLClient, ctx: Context) -> None:
    ops = PickupLocationOperations(client=graphql_client)
...
@pytest.mark.graphql
@pytest.mark.skip
def test_product_pickup_locations_multiple_ffcs(graphql_client: GraphQLClient, ctx: Context) -> None:
    ops = PickupLocationOperations(client=graphql_client)
...
@pytest.mark.graphql
@pytest.mark.skip
def test_product_pickup_locations_all_availability_types(graphql_client: GraphQLClient, ctx: Context) -> None:
    ops = PickupLocationOperations(client=graphql_client)
    locations = _get_locations(ops, _PRODUCT_MULTIPLE_FFC, ctx)
