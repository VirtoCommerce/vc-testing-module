...
@allure.feature("Checkout / Shipping method (E2E)")
@allure.title("Switch between FixedRate Ground and Air on single-page checkout")
def test_checkout_shipping_method_single_page(
    ...
    with allure.step("Navigate to the cart page and toggle to shipping mode"):
        cart_page.navigate()
    ...
    fixed_rate_ground = dataset_manager.find_shipping_method(
        code=_FIXED_RATE, option="Ground"
    )
    fixed_rate_air = dataset_manager.find_shipping_method(
        code=_FIXED_RATE, option="Air"
    )

    with allure.step(f"Select '{_FIXED_RATE_GROUND}' and verify shipping cost"):
        ...
        expect(cart_page.shipping_cost_label).to_contain_text(
            str(fixed_rate_ground["option"]["price"])
        )

    with allure.step(f"Select '{_FIXED_RATE_AIR}' and verify shipping cost"):
        cart_page.shipping_details_section.select_shipping_method(_FIXED_RATE_AIR)
        selected_shipping_method = (
            cart_page.shipping_details_section.find_selected_shipping_method(
                code=_FIXED_RATE_AIR
            )
        )
        expect(selected_shipping_method).to_be_visible()
        expect(cart_page.shipping_cost_label).to_contain_text(
            str(fixed_rate_air["option"]["price"])
        )
...
@allure.feature("Checkout / Shipping method (E2E)")
@allure.title("Switch between FixedRate Ground and Air on multi-step checkout")
def test_checkout_shipping_method_multi_step(
    ...
    with allure.step("Verify shipping page renders shipping method selector"):
        expect(shipping_page.shipping_details_section.root).to_be_visible()
        expect(
            shipping_page.shipping_details_section.shipping_method_selector
        ).to_be_visible()
    ...
    with allure.step(f"Select '{_FIXED_RATE_GROUND}' and verify shipping cost"):
        ...
        expect(shipping_page.shipping_cost_label).to_contain_text(
            str(fixed_rate_ground["option"]["price"])
        )
