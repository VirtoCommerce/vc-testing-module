...
from page_objects.frontend.pages import HomePage
...
def test_ship_to_selector_anonymous_user(
    ...
    with allure.step("Verify ship-to selector reflects the submitted address"):
        ...
        expect(home_page.top_header.ship_to_selector.selected_address_label).to_contain_text(
            str(TEST_ADDRESS.country_name)
        )
        expect(home_page.top_header.ship_to_selector.selected_address_label).to_contain_text(
            str(TEST_ADDRESS.region_name)
        )
        expect(home_page.top_header.ship_to_selector.selected_address_label).to_contain_text(str(TEST_ADDRESS.city))
        expect(home_page.top_header.ship_to_selector.selected_address_label).to_contain_text(
            str(TEST_ADDRESS.postal_code)
        )
