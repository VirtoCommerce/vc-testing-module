...
from page_objects.frontend.pages import CategoryPage, ProductPage
...
def test_category_configurable_product_card(global_settings: GlobalSettings, page: Page) -> None:
    category_page = CategoryPage(global_settings=global_settings, page=page, path=_CATEGORY_PATH)

    with allure.step(
        f"Navigate to category '{_CATEGORY_PATH}' and find a product card " f"'{_CONFIGURABLE_PRODUCT_CODE}'"
    ):
...
def test_category_configurable_product_navigation(global_settings: GlobalSettings, page: Page) -> None:
    category_page = CategoryPage(global_settings=global_settings, page=page, path=_CATEGORY_PATH)

    with allure.step(f"Navigate to category '{_CATEGORY_PATH}'"):
        category_page.navigate()

    with allure.step(f"Read the customize action target on card '{_CONFIGURABLE_PRODUCT_CODE}'"):
        product_card = category_page.scroll_to_product(sku=_CONFIGURABLE_PRODUCT_CODE)
    ...
    with allure.step("Verify the customize action opens the configurable product page"):
        product_page = ProductPage(global_settings=global_settings, page=page, path=target.lstrip("/"))
