...
from page_objects.frontend.components import EditAddressModal
from page_objects.frontend.pages import (
    CartPage,
...
@allure.feature("Checkout / Order creation (E2E)")
@allure.title("Place an order on single-page checkout")
def test_checkout_order_create_single_page(global_settings: GlobalSettings, page: Page) -> None:
    ...
    with allure.step(f"Select shipping method '{_FIXED_RATE_GROUND}'"):
        cart_page.shipping_details_section.select_shipping_method(code=_FIXED_RATE_GROUND)
        selected_shipping_method = cart_page.shipping_details_section.find_selected_shipping_method(
    ...
    with allure.step(f"Select payment method '{_MANUAL_PAYMENT_METHOD}'"):
        cart_page.payment_details_section.select_payment_method(code=_MANUAL_PAYMENT_METHOD)
        selected_payment_method = cart_page.payment_details_section.find_selected_payment_method(
    ...
    with allure.step("Place the order and verify the order-completed page"):
        cart_page.place_order_button.click()
        checkout_completed_page = CheckoutCompletedPage(global_settings=global_settings, page=page)
...
@allure.feature("Checkout / Order creation (E2E)")
@allure.title("Place an order on multi-step checkout")
def test_checkout_order_create_multi_step(global_settings: GlobalSettings, page: Page) -> None:
    cart_page = CartPage(global_settings=global_settings, page=page)
    ...
    with allure.step("Verify payment page renders billing/payment sections"):
        expect(payment_page.payment_details_section.root).to_be_visible()
        expect(payment_page.payment_details_section.billing_address_equals_shipping_checkbox).to_be_visible()
    ...
    with allure.step(f"Select payment method '{_MANUAL_PAYMENT_METHOD}' and continue to review"):
        payment_page.payment_details_section.select_payment_method(code=_MANUAL_PAYMENT_METHOD)
        selected_payment_method = payment_page.payment_details_section.find_selected_payment_method(
            code=_MANUAL_PAYMENT_METHOD
        )
        expect(selected_payment_method).to_be_visible()
    ...
    checkout_review_order_page = CheckoutReviewOrderPage(global_settings=global_settings, page=page)
    with allure.step("Place the order from the review page and verify completion"):
