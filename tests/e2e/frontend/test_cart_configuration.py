...
def _is_change_configured_mutation(response: Response) -> bool:
    ...
    return bool(post and "/graphql" in response.url and "ChangeCartConfiguredItem" in post)
...
def _add_configuration(product_page: ProductPage, memory_option: str) -> None:
    ...
    product_page.product_configuration_area.select_option(section_name=_MEMORY_SECTION_NAME, option_name=memory_option)
...
def test_cart_configuration_save_multiple(global_settings: GlobalSettings, page: Page) -> None:
    ...
    with allure.step("Expand each components list and verify the distinct memory options"):
        ...
        expect(cart_page.find_configured_line_item(_PRODUCT_ID, _MEMORY_OPTION_A).root).to_be_visible()
        expect(cart_page.find_configured_line_item(_PRODUCT_ID, _MEMORY_OPTION_B).root).to_be_visible()
...
def test_cart_configuration_remove(global_settings: GlobalSettings, page: Page) -> None:
    product_page = ProductPage(global_settings=global_settings, page=page, path=_PATH)
    cart_page = CartPage(global_settings=global_settings, page=page)
...
def test_cart_configuration_change_in_cart(global_settings: GlobalSettings, page: Page) -> None:
    product_page = ProductPage(global_settings=global_settings, page=page, path=_PATH)
    cart_page = CartPage(global_settings=global_settings, page=page)
