...
def test_product_configuration_add_to_cart(
    ...
    with allure.step("Verify the required configuration sections are present"):
        memory_section = product_page.product_configuration_area.find_section_by_name(
            name=_MEMORY_SECTION_NAME
        )
        expect(memory_section.root).to_be_visible()
        assert memory_section.is_required

        storage_section = product_page.product_configuration_area.find_section_by_name(
            name=_STORAGE_SECTION_NAME
        )
