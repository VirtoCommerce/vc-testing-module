...
from page_objects.pages import CartPage, ProductPage, SignInPage
    ...
    try:
        with GraphQLClient(auth=provider, global_settings=global_settings) as client:
            ...
            cart_ops.add_items_to_cart(
                ...
                items=[
                    CartItemInput(
                        product_id=_REGULAR_PRODUCT_ID, quantity=_REGULAR_QUANTITY
                    )
                ],
                currency_code=user_ctx.currency_code,
                culture_name=user_ctx.culture_name,
...
@allure.title(
    "Anonymous configured product merges into the signed-in cart holding a regular product"
)
def test_cart_merge_configured_and_regular(
    ...
    try:
        ...
        with allure.step(
            f"Seed the registered user's cart with regular product '{_REGULAR_PRODUCT_ID}'"
        ):
            _seed_user_cart(global_settings, user_ctx)

        with allure.step(
            "As anonymous user, configure a product and add it to the cart"
        ):
        ...
        with allure.step(
            "Open the cart and verify both the regular and configured products are present"
        ):
