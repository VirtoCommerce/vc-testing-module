...
def resolve_search_keyword(items: list[dict[str, Any]], case_id: str) -> str:
    ...
    if case_id == "whitespace_leading":
        partial = items[3]["name"].split()[0]
        return f" {partial}"

    if case_id == "whitespace_only":
        return "   "

    if case_id == "whitespace_double":
        for loc in items:
            if " " in loc["name"]:
                return loc["name"].replace(" ", "  ", 1)
        pytest.skip("No location name with a space found in GraphQL data")
