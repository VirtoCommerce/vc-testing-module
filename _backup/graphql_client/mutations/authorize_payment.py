from gql import gql
from graphql_client.types.authorize_payment_result_type import AuthorizePaymentResultType


class AuthorizePaymentMutation:
    def __init__(self, graphql_client):
        self.graphql_client = graphql_client

    def execute(self, variables: dict, return_fields: str = None) -> AuthorizePaymentResultType:
        query_string = f"""
            mutation authorizePayment($command: InputAuthorizePaymentType!) {{
                authorizePayment(
                    command: $command
                ) {{
                    {return_fields}
                }}
            }}
        """

        return self.graphql_client.execute(gql(query_string), variables)["authorizePayment"]
