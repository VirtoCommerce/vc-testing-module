...
class DatasetManager:
    ...
    _logger: FileLogger = FileLogger("dataset_manager", base_dir / "dataset_manager.log")
    ...
    def __init__(self) -> None:
        ...
        self._variable_resolvers: dict[VariableType, Callable[[str, dict[str, Any]], Any]] = {
    ...
    def _filter_requests(self, entities: list[str]) -> tuple[dict[str, SeedRequest], list[str]]:
        if not entities:
        ...
        found = {name: self.seed_requests[name] for name in entity_set if name in available}
    ...
    def _resolve_payload_item(self, variable_name: str, item: dict[str, Any]) -> Any:
        if not isinstance(item, dict):
            raise KeyError(f"PAYLOAD_ITEM requires dict payloads; got {type(item).__name__}")
        try:
            return item[variable_name]
        except KeyError as error:
            raise KeyError(f"PAYLOAD_ITEM property not found in payload item: {variable_name}") from error

    def _resolve_variable(self, variable_type: str, variable_name: str, item: dict[str, Any]) -> Any:
        try:
    ...
    def _interpolate_payload(self, value: Any, item: dict[str, Any]) -> Any:
        if isinstance(value, dict):
            return {key: self._interpolate_payload(nested, item) for key, nested in value.items()}
    ...
    def _interpolate_env_payload(self, value: Any) -> Any:
        if isinstance(value, dict):
            return {key: self._interpolate_env_payload(nested) for key, nested in value.items()}
        if isinstance(value, list):
    ...
    def load_requests(self) -> None:
            ...
            for file_path in file_paths:
                try:
                    with open(file_path, "r", encoding="utf-8") as file:
                        ...
                        seed_request.endpoint = self._interpolate_env_string(seed_request.endpoint)
                        seed_request.data = [self._interpolate_env_payload(item) for item in seed_request.data]
                        dataset_name = camelize(file_path.stem, uppercase_first_letter=False)
        ...
        self.seed_requests = dict[str, SeedRequest](sorted(seed_requests.items(), key=lambda item: item[1].priority))
    ...
    def seed(self, entities: list[str]) -> None:
        ...
        if absent_entities:
            self._console.print(f"[yellow]WARNING: Entities not found: {', '.join(absent_entities)}[/yellow]")
        ...
        def process(progress: Progress, task_id: int) -> None:
            ...
            def send_request(name: str, method: str, endpoint: str, body: Any) -> None:
                ...
                try:
                    ...
                    self._console.print(f"[red]ERROR: Failed to seed {name}: {url}[/red]")
