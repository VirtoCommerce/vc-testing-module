...
def test_e2e_add_product_to_cart_and_save_for_later(
    ...
    expect(line_item.element).not_to_be_visible(), f"Product {product['code']} should not be in cart"
    ...
    expect(saved_item.element).to_be_visible(), f"Product {product['code']} should be in saved for later"

    save_for_later_page.remove_line_item(product["code"])

    expect(saved_item.element).not_to_be_visible(), f"Product {product['code']} should not be in saved for later"
...
def test_e2e_move_product_from_saved_for_later_to_cart(
    ...
    cart_page.navigate()
    line_item = cart_page.get_line_item_by_sku(product["code"])
    expect(line_item.element).to_be_visible(), f"Product {product['code']} should be in cart"
