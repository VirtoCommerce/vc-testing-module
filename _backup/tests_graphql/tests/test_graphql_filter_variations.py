...
def test_filter_product_variations_by_stock(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    assert search_variations_result_all["totalCount"] == 4, "Total count of variations is not correct"
    assert search_variations_result_in_stock["totalCount"] == 3, "Total count of variations in stock is not correct"


@pytest.mark.graphql
@allure.feature("Filter product variations by price (GraphQL)")
def test_filter_product_variations_by_price(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
...
def test_filter_product_variations_by_property(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
