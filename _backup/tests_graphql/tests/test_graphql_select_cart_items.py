...
def test_select_cart_items(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    product_id_in_stock_1 = random.choice(
        [product for product in dataset["productInventories"] if product["inStockQuantity"] > 0]
    )["productId"]
    ...
    line_item_to_select = next(item for item in cart["items"] if item["productId"] == product_id_in_stock_2)
    ...
    selected_line_item = next(item for item in updated_cart["items"] if item["productId"] == product_id_in_stock_2)
    ...
    assert selected_line_item["productId"] == product_id_in_stock_2, "Selected line item product ID mismatch"
    assert selected_line_item["quantity"] == 2, "Selected line item quantity mismatch"


@pytest.mark.graphql
@allure.title("Select all cart items (GraphQL)")
def test_select_all_cart_items(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
