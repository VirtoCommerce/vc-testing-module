...
def test_product_specific_discount(
    ...
    discount_product = next(
        product for product in dataset["products"] if product["id"] == "product-acme-laptop-hp-omen-transcend-14"
    )
    ...
    line_item = next(line_item for line_item in cart["items"] if line_item["productId"] == product["id"])
    ...
    assert product["price"]["discountAmount"]["amount"] > 0, "Discount amount is not greater than 0"
    assert (
        product["price"]["list"]["amount"] - product["price"]["discountAmount"]["amount"]
        == product["price"]["actual"]["amount"]
    ), "Actual price is not correct"
    assert (
        line_item["discountTotal"]["amount"] == product["price"]["discountAmount"]["amount"]
    ), "Discount total is not correct"
    assert line_item["listPrice"]["amount"] == product["price"]["list"]["amount"], "List price is not correct"
    assert line_item["placedPrice"]["amount"] == product["price"]["actual"]["amount"], "Placed price is not correct"
...
def test_cart_subtotal_discount(
    ...
    culture = dataset["languages"][0]["allowedValues"][0]
    discount_product = next(
        product for product in dataset["products"] if product["id"] == "product-acme-laptop-lenovo-legion-9i-gen-10"
    )
