...
def test_success_catalog_browsing_as_anonymous_user(
    ...
    category_to_compare = next(
        category for category in categories_response["items"] if category["id"] == "category-acme-laptops"
    )
...
def test_unsuccess_catalog_browsing_as_anonymous_user(
    ...
    idx = next(i for i, s in enumerate(store["settings"]) if s["name"] == "Stores.AllowAnonymousUsers")
    print(f"{os.linesep}Setting 'Stores.AllowAnonymousUsers' found at index {idx}, disabling...", end=" ")
    webapi_client.patch(
