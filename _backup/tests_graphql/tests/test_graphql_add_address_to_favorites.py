...
def test_add_address_to_favorites(
    ...
    favorite_address = next(
        (address for address in organization["addresses"]["items"] if address["isFavorite"]),
        None,
    )
    ...
    assert favorite_address["id"] == added_address["id"], "Favorite address is not the added address"
