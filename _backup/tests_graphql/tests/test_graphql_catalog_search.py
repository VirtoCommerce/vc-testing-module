...
def test_catalog_search_by_product_full_name(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    assert product_to_search["name"] == searched_product["name"], "Product name does not match"
...
def test_catalog_search_by_product_sku(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    assert products_response["totalCount"] == 1, "Total count of products does not match"
    assert searched_product["code"] == product_to_search["code"], "Product SKU does not match"


@pytest.mark.graphql
@allure.title("Catalog search by product availability (GraphQL)")
def test_catalog_search_by_product_availability(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    print(
    ...
    assert (
        products_in_stock_only_response["totalCount"] < products_all_response["totalCount"]
    ), "Total count of products in stock only does not match"


@pytest.mark.graphql
@allure.title("Catalog search by product brand (GraphQL)")
def test_catalog_search_by_product_brand(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    assert products_response["totalCount"] == 7, "Total count of products does not match"


@pytest.mark.graphql
@allure.title("Catalog search by product price (GraphQL)")
def test_catalog_search_by_product_price(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
