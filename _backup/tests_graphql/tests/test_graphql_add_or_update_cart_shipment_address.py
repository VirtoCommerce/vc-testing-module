...
def test_add_cart_shipment_address(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    assert delivery_address["city"] == test_address["city"], "Delivery address city is not the same"
    assert (
    ...
    assert delivery_address["email"] == test_address["email"], "Delivery address email is not the same"
    assert delivery_address["firstName"] == test_address["firstName"], "Delivery address first name is not the same"
    assert delivery_address["lastName"] == test_address["lastName"], "Delivery address last name is not the same"
    assert delivery_address["line1"] == test_address["line1"], "Delivery address line 1 is not the same"
    assert delivery_address["phone"] == test_address["phone"], "Delivery address phone is not the same"
    assert delivery_address["postalCode"] == test_address["postalCode"], "Delivery address postal code is not the same"
    assert delivery_address["regionId"] == test_address["regionId"], "Delivery address region ID is not the same"
    assert delivery_address["regionName"] == test_address["regionName"], "Delivery address region name is not the same"


@pytest.mark.graphql
@allure.title("Update cart shipment address (GraphQL)")
def test_update_cart_shipment_address(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
