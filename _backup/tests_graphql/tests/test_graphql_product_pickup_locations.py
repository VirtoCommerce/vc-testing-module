...
def test_product_pickup_locations_main_ffc(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    locations_with_today = [loc for loc in result["items"] if loc.get("availabilityType") == "Today"]
...
def test_product_pickup_locations_ohio_ffc(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
...
def test_product_pickup_locations_multi_ffc(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
...
def test_product_pickup_locations_track_inventory_false(
    ...
    for location in result["items"]:
        assert (
            location.get("availabilityType") == "Today"
        ), f"Location {location['name']} should have Today availability, got {location.get('availabilityType')}"
...
def test_product_pickup_locations_european_berlin_billund(
    ...
    european_locations = [loc for loc in result["items"] if loc.get("address", {}).get("countryCode") in ["DEU", "DNK"]]

    european_with_stock = [loc for loc in european_locations if loc.get("availableQuantity", 0) > 0]

    assert len(european_with_stock) > 0, "No European locations have stock for this product"
...
def test_product_pickup_locations_three_ffcs(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    locations_with_stock = [loc for loc in result["items"] if loc.get("availableQuantity", 0) > 0]

    assert len(locations_with_stock) > 3, "Product in 3 FFCs should be available at many locations"

    us_locations = [loc for loc in locations_with_stock if loc.get("address", {}).get("countryCode") == "USA"]
    eu_locations = [loc for loc in locations_with_stock if loc.get("address", {}).get("countryCode") in ["DEU", "DNK"]]
...
def test_product_pickup_locations_pagination(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
...
def test_product_pickup_locations_availability_types(
    ...
    for avail_type in availability_types:
        assert avail_type in valid_types, f"Invalid availability type: {avail_type}"
