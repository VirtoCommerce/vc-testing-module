...
def test_add_cart_billing_address(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    assert billing_address["city"] == test_address["city"], "Billing address city is not the same"
    assert billing_address["countryCode"] == test_address["countryCode"], "Billing address country code is not the same"
    assert billing_address["countryName"] == test_address["countryName"], "Billing address country name is not the same"
    assert billing_address["email"] == test_address["email"], "Billing address email is not the same"
    assert billing_address["firstName"] == test_address["firstName"], "Billing address first name is not the same"
    assert billing_address["lastName"] == test_address["lastName"], "Billing address last name is not the same"
    assert billing_address["line1"] == test_address["line1"], "Billing address line 1 is not the same"
    assert billing_address["phone"] == test_address["phone"], "Billing address phone is not the same"
    assert billing_address["postalCode"] == test_address["postalCode"], "Billing address postal code is not the same"
    assert billing_address["regionId"] == test_address["regionId"], "Billing address region ID is not the same"
    assert billing_address["regionName"] == test_address["regionName"], "Billing address region name is not the same"


@pytest.mark.graphql
@allure.title("Update cart billing address (GraphQL)")
def test_update_cart_billing_address(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
