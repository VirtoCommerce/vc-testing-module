...
def test_unselect_cart_items(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
    ...
    line_item_to_unselect = next(item for item in cart["items"] if item["productId"] == product_id_in_stock_2)
    ...
    unselected_line_item = next(item for item in updated_cart["items"] if item["productId"] == product_id_in_stock_2)
...
def test_unselect_all_cart_items(config: Config, dataset: dict[str, Any], graphql_client: GraphQLClient):
