...
def test_get_shipment_costs(
    ...
    assert (
        cart["shipments"][0]["shipmentMethodOption"] == bopis_shipping_method["optionName"]
    ), "Shipment method option is not the same"
    assert cart["shippingTotal"]["amount"] == bopis_shipping_method["price"]["amount"], "Shipping total is not the same"
