...
@pytest.mark.e2e
def test_e2e_category_scroll_to_unexisting_product(
    config: Config, page: Page, dataset: dict[str, Any]
):
    print(
        f"{os.linesep}Running E2E test to scroll to unexisting product on category page...",
        end=" ",
    )
    ...
    product_card = category_page.scroll_to_product_card("unexisting-product-sku")

    assert product_card is None, "Product card should not be found"
